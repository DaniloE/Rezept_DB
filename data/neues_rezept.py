...
class rezepte_neu(QtGui.QMainWindow):
    def __init__(self, parent=None):
        ...
        self.setWindowTitle('Neues Rezept hinzufügen')
        ...
        self.rezept_name.setGeometry(70, 10, 250, 30)
        ...
        self.beschreibung_label.setText("Zubereitung:")
    ...
    def zutat_minus(self):
        ...
        if zutat and len(zutat) > 0:
            ...
            self.zutaten_liste_anzeige_model.clear()
            for key in self.zutaten_dict.keys():
                eintrag = str(self.zutaten_dict[key]).strip() + " " + str(key).strip()
                zutatenliste_item = QtGui.QStandardItem(eintrag)
                self.zutaten_liste_anzeige_model.appendRow(zutatenliste_item)
            self.zutaten_liste_anzeige.repaint()
    ...
    def save_rezept(self):
        ...
        name_vorhanden = self.db_connect.test_rezeptname(rezeptname)
        save_rezept_messagebox = QtGui.QMessageBox(self)
        save_rezept_messagebox.setGeometry(700, 500, 120, 100)
        if name_vorhanden == 0:
            return_insert = self.db_connect.saverezept(kategorie_liste, beschreibung, zutaten, rezeptname)
            if return_insert == 0:
                save_rezept_messagebox.setText("Rezept eingetragen")
                ...
                save_rezept_messagebox.show()
        elif name_vorhanden == 1:
            save_rezept_messagebox.setText("Bitte dem Rezept einen Namen geben bevor gespeichert werden kann")
            save_rezept_messagebox.show()
        elif name_vorhanden == 2:
            save_rezept_messagebox.setText(
                "Der Rezeptname existiert bereits in der Datenbank.\nBitte einen anderen wählen")
            save_rezept_messagebox.show()
        else:
            save_rezept_messagebox.setText("Es ist ein nicht erwarteter Fehler aufgetreten. Sieh zu...")
            save_rezept_messagebox.show()
